 ...
 */
	protected function _getDescriptionFromFirstLine(Model $model) {
        ...
        if ($line === "") {                 // try the second line if the first is empty (eg Natwest CSV)
            $line = rtrim(fgets($handle));
        }
        $line = rtrim($line, ',;');
        ...
        // LWK This is the place to add loading of new CSV processing behaviours in the future
        // for now using simple field name detections
        switch ($this->fields[0]) {
            case 'Transaction Date': // first field in TSB CSV
                $model->Behaviors->load('TSBCsv');
                break;
            case 'Date': // first field in Natwest CSV
                $model->Behaviors->load('NatwestCsv');
                break;
            default:
                break;
        }
